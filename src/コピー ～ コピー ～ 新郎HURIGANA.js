const WOMEN_HURIGANA_SW = function () {
  ...
  const women_name = getitem("photo", "G8");

  const women_name_kana = KATAKANA(women_name);

  const cell_write = function (value) {
    const urlss = SpreadsheetApp.getActiveSpreadsheet();
    var sheet = urlss.getSheetByName("photo");
    sheet.getRange("G7").setValue(value);
  };

  cell_write(women_name_kana);
};
